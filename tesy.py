...
def get_authorization_url(client_id, scope):
    ...
    This function takes your client ID and the desired access scope (e.g., "accesslink.read_all") as arguments and constructs the complete authorization URL.
    The URL directs users to Polar's login page where they can grant access to your application.
...
def get_access_token(client_id, client_secret, authorization_code):
    ...
    This function handles the exchange of the authorization code obtained from Polar's login flow for an access token.
    It also retrieves the user ID associated with the granted access.
...
def get_user_information(access_token, user_id):
    ...
    This function retrieves user data associated with the provided user ID.
    It requires a valid access token for authorization.
...
def retrieve_heart_rate_data(access_token, date):
    """
    Retrieves heart rate data for a specific date using Polar API.

    This function retrieves heart rate data for a user.
    It requires the access token and an optional date parameter (YYYY-MM-DD format).
...
if __name__ == "__main__":
    ...
    try:
        ...
        heart_rate_data = retrieve_heart_rate_data(access_token, date)
        if heart_rate_data:
